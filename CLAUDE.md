...
- `bookActivities/{id}.grouped` — **곁텍스트·RAFT를 모둠으로 진행**
  ...
  - **만들기 창의 방식 기본값은 '기본 모둠'**입니다(반에 기본 모둠이 있을 때 —
    `BookActivityForm`의 `defaultMode`). 수업이 대체로 늘 같은 모둠으로
    돌아가는데 활동마다 다시 짜면 같은 일을 되풀이하게 됩니다. 기본 모둠이
    없는 반에서는 고를 수 없으므로(빈 모둠만 생깁니다) 종류가 보통 하는
    방식으로 돌아갑니다 — 닿소리는 '활동 모둠', 곁텍스트·RAFT는 '개별 활동'.
