...
- `bookActivities/{id}/groups/{gId}/words` — 책방 닿소리 낱말
  ...
  - **교사 화면 왼쪽 모둠 목록에서 모둠원 이름은 각각 단추**입니다(한 줄에 셋).
    누르면 `ConsonantCanvas`에 `focusUid`가 내려가 **그 학생 낱말만** 남고,
    다시 누르면 모둠 판으로 돌아옵니다('내 판'의 거르기와 같은 방식).
    모둠 판은 낱말이 색으로만 갈려 있어 한 사람 것을 눈으로 좇기 어려웠습니다.
    카드 전체를 단추로 두지 않는 이유: 단추 안에 단추를 넣을 수 없어서
    모둠 이름 줄(`.book-rail-pick`)과 이름들(`.book-rail-person`)이 따로 눌립니다.
