...
- **체크 목록**(`checkList`) — 수업 메모 툴바의 다섯째. 할 일을 적어 두고
  나중에 지우는 자리라 글머리 기호만으로는 모자랐습니다.
  - 저장 모양은 `<ul class="checklist">` + `<li>`이고, 켠 줄은 `class="done"`
    입니다. **`<input type="checkbox">`를 쓰지 않습니다** — 그러려면 정화기
    (`sanitizeHtml`)에 태그 하나와 속성 둘을 새로 열어야 하는데, 남의 글이
    그대로 뿌려지는 자리가 여럿이라(질문·답변·카드·메모) 여는 문은 적을수록
    좋습니다. class 두 값을 더하는 쪽이 훨씬 좁고, 상자는 CSS가 그립니다.
    한 줄 미리보기(`stripHtml`)에 글자가 남는 것도 이쪽뿐입니다.
  - **켜고 끄는 것은 읽는 자리에서** 합니다(지난 메모 패널·달력의 펼친 메모).
    켜려고 '수정'으로 들어갔다 나오게 하면 체크 목록을 둔 뜻이 없습니다.
    누르면 그 메모 문서의 HTML을 고쳐 바로 씁니다(`toggleChecklistItem`).
  - **네모를 누른 자리는 왼쪽 22px**입니다. 글자를 누르면 안 켜집니다 —
    읽다가 무심코 눌러 할 일이 지워지면 안 됩니다. 그 값이 세 곳에 있으니
    (`RichTextEditor`의 `CHECK_HIT` · `LessonMemoModal`의 `CHECK_HIT` ·
    `ul.checklist > li`의 padding) 고칠 때는 함께 고치세요.
  - 목록 자체는 브라우저(`execCommand`)가 만들므로 Enter로 줄이 이어지고,
    표시는 그 `<ul>`에만 붙습니다 — 새 `<li>`는 저절로 체크 줄이 됩니다.
