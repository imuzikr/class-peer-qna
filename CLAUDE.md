...
- 곁텍스트 활동 화면의 **모둠 고르는 줄 끝**(마지막 모둠 뒤)에 `전광판`.
  `components/ParatextProgressBoard.jsx`가 **여덟 단계 × 반 전체**를 한
  격자로 엽니다 — 한 줄이 단계 하나, 한 칸이 학생 한 명(가로 배치).
  공부방의 `StudyProgressBoard`를 옮긴 것이라 **격자도 CSS도 그대로**
  씁니다(`.progress-grass`·`.grass-*`·`.progress-tip`) — 두 방의 전광판이
  다른 모양이면 교사가 같은 것을 두 번 익혀야 합니다.
...
- **칸 색은 학생 카드의 여덟 네모(`.paratext-mark`)와 같은 값**입니다 —
...
- **붙어 있는 칸(sticky)의 바탕은 그 모달의 바탕과 같아야 합니다.** 공부방의
  ...
  클래스 `.reading-board` 아래에서 흰색으로 덮습니다.
...
- **읽는 문서가 늘지 않습니다** — 단계별 인원도 완성 인원도 `ParatextBoard`가
  이미 구독해 둔 `entries`로 셉니다.
