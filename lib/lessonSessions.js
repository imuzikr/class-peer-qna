...
//
// 나누는 규칙
//   · 한국 시간 08:00 이상·16:00 미만 출석만 사용합니다.
//   · 이 범위 안의 첫 출석 = 수업 시작
...
const MIN = 60 * 1000;
const KST_OFFSET = 9 * 60 * MIN;
const SCHOOL_START_HOUR = 8;
const SCHOOL_END_HOUR = 16;

// UTC+9 시계를 UTC 메서드로 읽어 브라우저/서버의 현지 시간대 영향을 없앱니다.
function inSchoolHours(time) {
  if (!Number.isFinite(time)) return false;
  const hour = new Date(time + KST_OFFSET).getUTCHours();
  return hour >= SCHOOL_START_HOUR && hour < SCHOOL_END_HOUR;
}

function schoolDayEnd(time) {
  const clock = new Date(time + KST_OFFSET);
  clock.setUTCHours(SCHOOL_END_HOUR, 0, 0, 0);
  return clock.getTime() - KST_OFFSET;
}
...
function timeOf(rec) {
  const value = rec?.attendedAt ?? rec?.createdAt;
...
export function sessionAt(sessions, time) {
  ...
  if (!inSchoolHours(t)) return null;
...
// 과일을 준 시각이 수업의 앞·가운데·뒤 어디에 몰렸나 — RewardTiming이 그립니다.
// 원래 그 컴포넌트의 useMemo 안에 있던 셈을 그대로 옮겼습니다. 시험이 소스를
// 글자 위치로 잘라 돌리던 것을, 이 함수를 곧바로 부르게 하려고요.
// 수업 시간을 추정할 출석 기록이 없으면 null.
export function rewardTimingStat(events = [], attendance = []) {
