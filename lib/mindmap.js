...
export function radialSpawn(nodes, parentId) {
  ...
  if (depth === 0) {
    ...
    const base = Math.atan2(py, px); // 뿌리에서 부모로 뻗은 방향
    const step = 0.5;
...
export function computeTreeLayout(nodes, { colGap = 210, rowGap = 62 } = {}) {
  ...
  const roughLevels = levelMap(nodes);
  const gapByDepth = new Map();
  for (const n of nodes) {
    if (n.parentId === null) continue;
    const parentDepth = roughLevels.get(n.parentId) ?? 0;
    const labelGap = gapForEdgeLabel(n.edgeLabel);
    gapByDepth.set(
      parentDepth,
      Math.max(gapByDepth.get(parentDepth) ?? colGap, labelGap)
    );
  }
  function xForDepth(depth) {
    let x = 0;
    for (let d = 0; d < depth; d += 1) {
      x += gapByDepth.get(d) ?? colGap;
    }
    return x;
  }
  ...
  function walk(node, depth) {
    ...
    pos.set(node.id, { x: xForDepth(depth), y });
    return y;
  }
...
export function layoutPositions(map) {
  ...
  return new Map(
    map.nodes.map((n) => [n.id, { x: Number.isFinite(n.x) ? n.x : 0, y: Number.isFinite(n.y) ? n.y : 0 }])
  );
}
