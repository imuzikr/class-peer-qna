...
// '이 노트를 읽고 과일을 줬나' — 기록 관리 수업 노트 탭의 카드 색.
// -------------------------------------------------------------
...
// 위 판정에 '오늘'만 더한 것입니다(두 화면이 같은 칸 하나를 봅니다).
// 카드 색과 달리 날짜를 함께 보는 까닭: 단추가 답하는 물음은 '오늘 눌렀는가'
// 라서, 지난주에 준 노트를 오늘 다시 열어 읽을 때는 또 줄 수 있어야 합니다.
// (`toDate`는 빈 값에 '지금'을 돌려주므로 **원본 값**으로 먼저 걸러야 합니다 —
// 그냥 넘기면 안 준 노트가 전부 '오늘 줌'이 됩니다.)
export function isCornellRewardedToday(note) {
  if (!isCornellRewarded(note)) return false;
