...
export async function deleteAnswer(questionId, answerId) {
  ...
  const target = mock.questions.find((q) => q.id === questionId);
  if (target && target.answerCount > 0) target.answerCount -= 1;
  notify(
    mockListeners.answers.get(questionId) ?? new Set(),
    sortByOldest(mock.answers[questionId])
  );
...
// '이 노트를 읽고 과일을 줬나' — 교사 화면의 자리 색을 가르는 판정 **한 곳**.
// 피드백을 쓴 날과 과일을 준 날이 **같아야** 합니다: 9월 14일 노트를 15일에
// 읽고 그날 과일을 줬다면 15일에 준 것이 맞고, 그 뒤 다른 날 다른 화면에서
// 준 과일은 이 노트에 대한 것이 아닙니다.
// (`toDate`는 빈 값에 '지금'을 돌려주므로 **원본 값**으로 먼저 봅니다)
export function isCornellRewarded(note) {
  if (!note?.feedbackAt || !note?.rewardedAt) return false;
  return todayDateKey(toDate(note.feedbackAt)) === todayDateKey(toDate(note.rewardedAt));
}

// '오늘 이 노트에 과일을 줬나' — 수업 노트 창의 과일 단추를 걷는 판정.
// -------------------------------------------------------------
// 묻는 것이 **'오늘 눌렀는가' 하나뿐**이라 도장 날짜만 봅니다. 위
// `isCornellRewarded`(카드 색)와 갈라 두는 까닭은 그쪽이 '피드백을 쓴 날과
// 같은 날인가'까지 함께 보기 때문입니다 — 피드백보다 과일을 먼저 주는 흔한
// 차례에서 거짓이 되어, 그 값으로 단추를 걷으면 **방금 줬는데 단추가 그대로**
// 남습니다.
// 날마다 새로 판정하므로 지난주에 준 노트를 오늘 다시 열면 또 줄 수 있습니다.
// (`toDate`는 빈 값에 '지금'을 돌려주므로 **원본 값**으로 먼저 봅니다 —
// 그냥 넘기면 안 준 노트가 전부 '오늘 줌'이 됩니다.)
export function isCornellRewardedToday(note) {
  if (!note?.rewardedAt) return false;
  return todayDateKey(toDate(note.rewardedAt)) === todayDateKey();
}
