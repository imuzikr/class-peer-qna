...
// [교사] 활동 만들기 — groupMode에 따라 빈 모둠을 미리 만들어 둡니다.
export async function addBookActivity(
  user,
  { classId, title, topic, groupMode = "teacher", groupCount = 4, maxPerGroup = 6 }
) {
  const data = {
    classId,
    type: "consonant",
  ...
  if (isFirebaseConfigured) {
    ...
    // 자유 구성은 학생이 들어올 빈 모둠이 먼저 있어야 합니다.
    if (data.groupMode === "free") {
      const batch = writeBatch(db);
  ...
  if (data.groupMode === "free") {
    for (let i = 1; i <= groupCount; i++) {
      mock.bookGroups.push({
        id: `group_${i}`, activityId: id, groupIndex: i, groupName: `${i}모둠`,
