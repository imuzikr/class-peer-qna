...
// resolved=false 로 되돌릴 때는 reflectionPending도 함께 초기화합니다.
export async function setQuestionResolved(questionId, resolved) {
  const patch = resolved
    ? { resolved: true }
    : { resolved: false, reflectionPending: false };
  if (isFirebaseConfigured) {
    await updateDoc(doc(db, "questions", questionId), patch);
    return;
  }
  ...
  if (target) {
    ...
    if (!resolved) target.reflectionPending = false;
  }
  notify(mockListeners.questions, sortByNewest(mock.questions));
}

// "나중에 쓸게요" — 해결은 되지만 회고를 미룬 상태로 표시합니다.
// 카드 목록과 상세 화면에 대기 배지가 붙어 학생 본인과 교사가 인지할 수 있습니다.
export async function setQuestionResolvedLater(questionId) {
  if (isFirebaseConfigured) {
...
//   reflection: { learned, next, authorId, authorName, authorEmoji, createdAt }
export async function addReflection(user, questionId, { learned, next }) {
  const reflection = {
    learned: learned ?? "",
    next: next ?? "",
    authorId: user.uid,
