...
// -------------------------------------------------------------
// 주간 질문대장 (Weekly Question Champions)
// -------------------------------------------------------------
// 지난 7일간 질문을 많이 올린 상위 7명을 랜딩 화면 하단에 보여 줍니다.
// 실서비스: Cloud Function(weeklyTopQuestioners)이 매주 월요일 오전 8시에
// 미리 집계해 둔 공개 문서 stats/weeklyQuestioners 를 구독합니다.
// (로그인 전에도 읽어야 하므로 규칙에서 이 문서만 공개 읽기를 허용)
// 콜백에 [{ authorName, authorEmoji, count }] 배열(내림차순)을 전달합니다.
export function subscribeWeeklyQuestioners(callback) {
  if (isFirebaseConfigured) {
    return onSnapshot(
      doc(db, "stats", "weeklyQuestioners"),
      (snap) => callback(snap.exists() ? snap.data().top ?? [] : []),
      () => callback([]) // 규칙/네트워크 오류 시 조용히 빈 목록
    );
  }
  // 데모 모드 — mock.questions에서 즉석 집계 (질문이 바뀔 때마다 갱신)
  const compute = () => {
    const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
    const byUser = new Map();
    personalizeDemo(sortByNewest(mock.questions)).forEach((q) => {
      const t = q.createdAt?.getTime?.() ?? Date.now();
      if (t < weekAgo || !q.authorId) return;
      const cur = byUser.get(q.authorId) ?? {
        authorName: q.authorName || "익명",
        authorEmoji: q.authorEmoji || "🙂",
        count: 0,
      };
      cur.count += 1;
      byUser.set(q.authorId, cur);
    });
    return [...byUser.values()]
      .sort((a, b) => b.count - a.count)
      .slice(0, 7);
  };
  const wrapper = () => callback(compute());
...
export async function addStudyCard(user, boardId, { title = "", content, imageUrl = null, attachments = [] }) {
  ...
  if (existing) {
    ...
    mock.studyCards.push({
      id: isTeacherUser ? `card${++mockSeq}_m` : user.uid,
      ...data,
      createdAt: new Date(),
      updatedAt: new Date(),
    });
