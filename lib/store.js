...
// 이번 주 시작(가장 최근 월요일 08:00 KST)을 epoch millis로 — 데모용
function weekStartMillis() {
  ...
  let start = kstMidnightUtc - daysSinceMon * 86400000 + 8 * 3600000;
  if (start > nowUtc) start -= 7 * 86400000;
  return start;
}

// authorId별로 개수를 세어 상위 5명 [{authorName,authorEmoji,count}] 반환.
// 이번 주(월요일 08:00 이후) 기록만 집계 — 매주 월요일 초기화.
...
export async function composeStudyGroups(user, boardId, groups) {
  if (isFirebaseConfigured) {
...
export async function addStudyBoard(
  ...
  const data = {
    ...
    keywords: keywords ?? [],
    activityType,
    viewMode: "private", // 기본값: 나만 보기(모둠 보드는 '자기 모둠만')
    editMode: "open",
    createdBy: user.uid,
  };
  if (isFirebaseConfigured) {
...
// 본인(학생)이 받은 과일 개수 구독 — 상단바 뱃지용.
// 소속 반(memberships)을 먼저 구독해 그 반의 rewards/{classId}_{uid} 문서를
// 각각 직접 구독합니다. 대부분 반 1개뿐이라 합계는 그 반의 개수와 같지만,
// 여러 반에 속한 경우에도 자연스럽게 합산됩니다.
export function subscribeMyRewardCount(uid, callback) {
  if (!uid) { callback(0); return () => {}; }
  const byClass = {};
  let unsubRewards = [];
  const emit = () => callback(Object.values(byClass).reduce((s, n) => s + n, 0));

  const unsubMemberships = subscribeMyMemberships(uid, (memberships) => {
    unsubRewards.forEach((u) => u());
    unsubRewards = [];
    Object.keys(byClass).forEach((k) => delete byClass[k]);

    const classIds = [...new Set(memberships.map((m) => m.classId))];
    if (classIds.length === 0) { emit(); return; }

    if (isFirebaseConfigured) {
      unsubRewards = classIds.map((classId) =>
        onSnapshot(
          doc(db, "rewards", `${classId}_${uid}`),
          (snap) => { byClass[classId] = snap.exists() ? (snap.data().count ?? 0) : 0; emit(); },
          () => { byClass[classId] = 0; emit(); }
        )
      );
      return;
    }
    ...
    const mockEmit = () => {
      classIds.forEach((classId) => {
        ...
        byClass[classId] = r?.count ?? 0;
      });
      emit();
    };
    mockListeners.rewards.add(mockEmit);
    unsubRewards = [() => mockListeners.rewards.delete(mockEmit)];
    mockEmit();
  });
    ...
    return onSnapshot(
      ...
      (snap) => {
        const list = snap.docs.map((d) => {
          const data = d.data();
          // 예전 가입 계정: 실명 칸에 "30105 홍길동"(워크스페이스 계정 이름)이
