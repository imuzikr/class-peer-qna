...
// ─── 관리자 전용: 학생 프로필 수정 ───────────────────────────────
// 해당 uid로 작성된 모든 questions·answers·studyCards의 author 필드를 갱신합니다.
export async function updateStudentProfile(uid, { name, emoji, realName }) {
  if (isFirebaseConfigured) {
    const batch = writeBatch(db);
    const qSnap = await getDocs(query(collection(db, "questions"), where("authorId", "==", uid)));
    qSnap.docs.forEach((d) => batch.update(d.ref, { authorName: name, authorEmoji: emoji, authorRealName: realName }));
    const aSnap = await getDocs(query(collectionGroup(db, "answers"), where("authorId", "==", uid)));
    aSnap.docs.forEach((d) => batch.update(d.ref, { authorName: name, authorEmoji: emoji, authorRealName: realName }));
    const cSnap = await getDocs(query(collection(db, "studyCards"), where("authorId", "==", uid)));
    cSnap.docs.forEach((d) => batch.update(d.ref, { authorName: name, authorEmoji: emoji, authorRealName: realName }));
    await batch.commit();
    return;
  }
  mock.questions = mock.questions.map((q) =>
    q.authorId === uid ? { ...q, authorName: name, authorEmoji: emoji, authorRealName: realName } : q
  );
  Object.keys(mock.answers).forEach((qid) => {
    mock.answers[qid] = mock.answers[qid].map((a) =>
      a.authorId === uid ? { ...a, authorName: name, authorEmoji: emoji, authorRealName: realName } : a
    );
  });
  mock.studyCards = (mock.studyCards ?? []).map((c) =>
    c.authorId === uid ? { ...c, authorName: name, authorEmoji: emoji, authorRealName: realName } : c
  );
  notifyQuestions();
}
