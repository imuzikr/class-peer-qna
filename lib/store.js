...
// (Firestore는 정렬 필드가 없는 문서를 건너뜁니다). 50건만 받아 오므로
// 정렬은 화면에서 하는 편이 안전하고 색인도 늘지 않습니다.
...
export async function addLessonMemo(classId, user, text, date = null) {
...
export async function updateLessonMemo(classId, memoId, text, date = null) {
  const body = String(text ?? "").trim().slice(0, LESSON_MEMO_MAX);
...
//
// 그래서 한 번만 읽고 반별로 쥐고 있습니다(달력을 열 때 처음 한 번).
// 전체 화면의 '새로고침' 버튼이 이 캐시를 버립니다. 지금 보고 있는 날짜의
// 숫자는 화면이 이미 실시간으로 들고 있으므로(subscribeAllKwl) 화면에서
// 그 값으로 덮어씁니다 — 방금 쓴 학생이 달력에 바로 보이도록.
const _kwlDaysCache = new Map(); // classId -> { 'YYYY-MM-DD': 쓴 사람 수 }
...
export async function fetchKwlDays(classId, { force = false } = {}) {
  if (!classId) return {};
  if (!force && _kwlDaysCache.has(classId)) return _kwlDaysCache.get(classId);
  let rows;
  if (isFirebaseConfigured) {
    const snap = await getDocs(
  ...
  _kwlDaysCache.set(classId, days);
  return days;
}
...
// 한 친구와 주고받은 메모 — 오래된 것이 앞(대화처럼 위에서 아래로).
//
// **질의를 둘로 나눕니다**(보낸 것 / 받은 것). pairKey 하나만 걸면
// Firestore가 거부합니다 — list 규칙은 결과 문서가 아니라 **질의 자체**로
// 판정하므로, `resource.data.toUid == uid()`를 질의가 증명해 주지 못하면
// "Property toUid is undefined on object"로 떨어집니다(실측). 책방 동료
// 평가(subscribeMyPeerReviews)를 둘로 나눈 것과 같은 이유입니다.
// 등호 둘씩이라 복합 색인은 여전히 필요 없습니다.
export function subscribeGroupMemoThread(classId, myUid, otherUid, callback) {
  if (!classId || !myUid || !otherUid) { callback([]); return () => {}; }
  const key = memoPairKey(myUid, otherUid);
  if (isFirebaseConfigured) {
    let sent = [];
    let got = [];
    const emit = () => {
    ...
    const watch = (field, set) =>
      onSnapshot(
        query(collection(db, "groupMemos"), where("pairKey", "==", key), where(field, "==", myUid)),
  ...
  const emit = () => {
    const list = mock.groupMemos
      .filter((m) => m.pairKey === key && m.classId === classId)
