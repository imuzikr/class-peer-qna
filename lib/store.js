...
export function lessonMemoProgress(memo) {
  ...
  const pages = raw && !/^(p\.?|쪽|page)/i.test(raw) ? `p.${raw}` : raw;
  return { topic, pages, label: [topic, pages].filter(Boolean).join(" · ") };
}
...
export const LESSON_PAGES_MAX = 30;

function progressFields(extra) {
  return {
    topic: String(extra?.topic ?? "").trim().slice(0, LESSON_TOPIC_MAX),
    pages: String(extra?.pages ?? "").trim().slice(0, LESSON_PAGES_MAX),
  };
}
