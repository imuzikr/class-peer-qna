...
// 고른 하루의 지급분 — '기록 관리'의 수업 노트 탭이 '이 학생에게 오늘 과일을
// 줬나'를 카드 색으로 보여 주는 데 씁니다.
// -------------------------------------------------------------
// 위 '오늘' 함수와 나누지 않고 하나로 합칠 수도 있지만, 그쪽은 자리표가
// 늘 켜 두는 구독이라 '오늘'이 못 박혀 있는 편이 실수가 적습니다.
// 범위 조건 하나뿐이라 **복합 색인이 필요 없습니다**(등호 필터와 섞지 않음).
export function subscribeRewardEventsOn(classId, dateKey, callback) {
  if (!classId || !dateKey) {
    callback([]);
    return () => {};
  }
  ...
  if (isFirebaseConfigured) {
    return onSnapshot(
      ...
      (snap) => callback(sortByAt(snap.docs.map((doc2) => ({ id: doc2.id, ...doc2.data() })))),
      () => callback([])
    );
  }
  if (!mock.rewardEvents) mock.rewardEvents = [];
