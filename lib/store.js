...
// 한 줄 정리(회고) 저장 — "해결됐어요" 시점에 남기는 이해의 전환점.
// 질문 문서에 reflection 맵으로 붙여, 상세 화면에서 모두가 볼 수 있게 합니다.
// (나중에 복습 카드/관련 질문 묶음 기능이 이 데이터를 재료로 씁니다.)
...
export async function addReflection(user, questionId, { learned, next }) {
  ...
  if (isFirebaseConfigured) {
    await updateDoc(doc(db, "questions", questionId), {
      reflection: { ...base, createdAt: serverTimestamp() },
    });
    return;
  }
  const target = mock.questions.find((q) => q.id === questionId);
  if (target) target.reflection = { ...base, createdAt: new Date() };
  notify(mockListeners.questions, sortByNewest(mock.questions));
}
