...
// deleteStudentAccount(purgeStudentData)가 생기기 전에 탈퇴한 학생들의
// 흔적(과일 기록·반 소속 등)이 프로필 없이 남아 있는 것을 한 번에 정리
// — 최고 관리자 전용, 서버 함수 sweepOrphanedStudentData 호출.
export async function sweepOrphanedStudentData() {
  const res = await httpsCallable(functions, "sweepOrphanedStudentData")({});
  return res?.data ?? { swept: 0, warnings: [] };
}
