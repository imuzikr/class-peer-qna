...
// 활동 종류 — 'consonant'는 모둠 협동, 'paratext'는 개인 활동입니다.
// paratext는 모둠 대신 학생 1명당 문서 1개(entries/{uid})를 씁니다.
export const BOOK_ACTIVITY_TYPES = ["consonant", "paratext"];
...
export async function addBookActivity(
  ...
  // 곁텍스트 읽기는 개인 활동이라 모둠을 만들지 않습니다.
  const kind = BOOK_ACTIVITY_TYPES.includes(type) ? type : "consonant";
  if (kind === "paratext") groupCount = 0;

  const nameAt = (i) => (groupNames[i - 1] || "").trim() || `${i}모둠`;
  const data = {
    classId,
    type: kind,
    title: (title || "").trim() || (kind === "paratext" ? "곁텍스트 읽기" : "닿소리 채우기"),
    topic: (topic || "").trim(),
