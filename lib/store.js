...
// 교사가 주제어(도서명)를 비워 둘 수 있는 활동 — 학생이 자기 화면에서
// 직접 적습니다(entries/{uid}.topic). 학생마다 읽는 책이 다를 수 있는
// 활동만 여기 넣습니다: 곁텍스트 읽기는 제가 고른 책을 훑어보는 활동이고,
// RAFT는 읽은 것을 바탕으로 각자 글을 씁니다.
//
// KWLS·마인드맵은 뺐습니다 — 반이 같은 주제를 놓고 아는 것/알고 싶은 것을
// 모으는 활동이라, 학생마다 주제가 다르면 교사 화면의 모아보기가 의미를
// 잃습니다. 닿소리는 판 자체에 주제어를 적는 길이 따로 있습니다.
export const BOOK_STUDENT_TOPIC_TYPES = ["paratext", "raft"];
  ...
  const emit = () =>
    callback(
      mock.bookActivities
        ...
        // 최신순 — Firebase 분기와 같은 차례로 넘깁니다(목록이 이 순서에 기댑니다)
        .slice()
        .sort((a, b) => toDate(b.createdAt) - toDate(a.createdAt))
    );
