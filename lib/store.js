...
const mockListeners = {
  ...
  kwl: new Map(),        // `${classId}_${date}` -> Set(callback)
};
...
// =============================================================
// KWL — 수업 단위(classId + userId + date) 1인 1항목
// =============================================================
...
// 내 KWL 구독
export function subscribeMyKwl(classId, userId, date, callback) {
  if (isFirebaseConfigured) {
    return onSnapshot(doc(db, "kwl", `${classId}_${userId}_${date}`), (snap) => {
  ...
  const wrapped = (entries) =>
    callback(entries.find((e) => e.userId === userId) ?? null);
...
// KWL 항목 추가 (매 저장마다 새 문서 생성, 저장 후 입력창 초기화 목적)
...
// KWL 저장 (없으면 생성, 있으면 덮어씀)
export async function saveKwl(classId, user, date, { K = "", W = "", L = "" }) {
  const data = {
    classId,
    userId: user.uid,
    date,
  ...
  if (isFirebaseConfigured) {
    await setDoc(
      doc(db, "kwl", `${classId}_${user.uid}_${date}`),
      { ...data, updatedAt: serverTimestamp() },
      { merge: true }
    );
    return;
  }
  const idx = mock.kwl.findIndex(
    (e) => e.classId === classId && e.userId === user.uid && e.date === date
  );
  if (idx >= 0) {
    mock.kwl[idx] = { ...mock.kwl[idx], K, W, L, updatedAt: new Date() };
  } else {
    mock.kwl.push({ id: `kwl_${++mockSeq}`, ...data });
  }
  notifyKwl(classId, date);
