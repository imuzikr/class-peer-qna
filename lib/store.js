...
import { QUESTION_TAG_KEYS, QUESTION_NOTE_MAX } from "./questionTags";
import {
  emptyKwlsAnswers,
...
// 동료 평가는 서식 있는 글이라 저장 전에 한 번 걸러 냅니다(스크립트·onclick 등)
...
// ── 전광판 (classes/{cId}.marquee) ─────────────────────────
// 교사가 반 학생을 칭찬·격려하는 짧은 글. 상단바에 돌아가며 섭니다
// ('이달의 주니어 개발자 10101 홍길동').
//
// **반 문서에 담습니다.** 새 컬렉션을 만들면 보안 규칙을 새로 쓰고 시험을
// 돌려 배포해야 하는데, `classes/{cId}`는 읽기가 이미 `signedIn()`이고 교사
// 수정에 필드 화이트리스트가 없어 **규칙을 한 줄도 안 건드립니다**
// (파이썬 실행기의 `pyTarget`과 같은 길).
//
...
export function marqueeOf(cls) {
  return Array.isArray(cls?.marquee)
    ? cls.marquee.filter((m) => m && String(m.text ?? "").trim())
    : [];
}

export async function setClassMarquee(classId, list) {
  if (!classId) return;
  const value = (list ?? [])
    .map((m) => ({
    ...
    .slice(0, MARQUEE_MAX);
  if (isFirebaseConfigured) {
    await updateDoc(doc(db, "classes", classId), { marquee: value });
    return;
  }
  ...
  if (i >= 0) mock.classes[i] = { ...mock.classes[i], marquee: value };
...
// -------------------------------------------------------------
// `{ boardId, actIndex, at }`. 교사가 수업 모드에서 활동 하나를 배포하면 그
// 반 학생의 수업 노트 서랍에 '활동' 탭이 서고, 학생이 거기서 씁니다.
//
...
//
// `at`(배포한 시각)이 필요한 이유: 학생이 서랍을 닫아 둔 뒤에도 **새로 배포된
// 것**은 다시 열려야 합니다. 같은 활동을 두 번 배포하는 일도 있어
// (`boardId`·`actIndex`가 그대로라) 시각이 없으면 두 번째를 알아채지 못합니다.
//
  ...
  const value =
    target && target.boardId
      ? {
          ...
          // 배열이 아니라 필드라 serverTimestamp()를 쓸 수는 있지만, 학생
          // 화면이 '새 배포인가'를 곧바로 견주어야 해서 서버 답을 기다리지
          // 않는 값으로 둡니다(전광판의 `at`과 같은 이유).
          at: Date.now(),
        }
      : null;
  if (isFirebaseConfigured) {
...
// 반 문서에서 배포된 활동을 읽습니다 — 모양이 어긋난 값은 '없음'으로 봅니다.
...
// 카드 **한 장**만 읽기 — 파이썬 실행기의 '활동으로 보내기'가 씁니다.
// 이어 붙이려면 지금 든 내용을 알아야 하는데, 그 한 번을 위해 보드의 카드를
// 통째로 받을 이유가 없습니다(반 전체면 스물몇 건). 문서 ID가 학생 uid라
// 곧바로 집을 수 있습니다. 없으면 null — 아직 한 번도 안 쓴 카드입니다.
export async function fetchStudyCardOnce(boardId, cardId) {
  if (!boardId || !cardId) return null;
  if (isFirebaseConfigured) {
    const snap = await getDoc(doc(db, "studyBoards", boardId, "cards", cardId));
    return snap.exists() ? { id: snap.id, ...snap.data() } : null;
  }
  return (
    mock.studyCards.find((c) => c.boardId === boardId && c.id === cardId) ?? null
  );
}
...
export async function duplicateStudyBoard(board, targetClassId, user) {
  const data = {
    ...
    //
    // 잠금은 **옮기지 않고 전부 잠근 채로** 시작합니다. 원본의 잠금은 그 반의
    // 진도인데, 받는 반은 아직 진도가 0입니다. 예전에는 그대로 복사해서,
    // 활동을 다 열어 둔 반에서 복제하면 새 반에도 여덟 개가 한꺼번에 열린
    // 채로 도착했습니다. 수업 중 '활동 열기'로 하나씩 여는 흐름과 맞춥니다.
    activityLocks: (board.activities ?? []).map(() => true),
...
export async function addStudentReward(classId, uid, delta = 1, identity = null) {
  ...
  const before = existing?.count ?? 0;
  const next = clamp(before + delta);
  if (existing) Object.assign(existing, { count: next, ...extra });
...
export async function saveCornellNote(classId, user, date, values = {}) {
  ...
  if (!mock.cornellNotes) mock.cornellNotes = [];
  const found = mock.cornellNotes.find((n) => n.id === id);
  if (found) Object.assign(found, data, { updatedAt: new Date() });
  else mock.cornellNotes.push({ id, ...data, createdAt: new Date(), updatedAt: new Date() });
...
export async function saveCornellFeedback(classId, noteId, text, user) {
  ...
  if (!mock.cornellNotes) mock.cornellNotes = [];
  const found = mock.cornellNotes.find((n) => n.id === noteId);
  if (found) Object.assign(found, { feedback, feedbackAt: new Date(), feedbackBy: user?.uid ?? "" });
  mockListeners.cornellNotes?.forEach((cb) => cb());
}
...
export async function addBookActivity(
  ...
  const grouped =
    ...
    groupMode !== "solo";
  if (BOOK_SOLO_TYPES.includes(kind) && !grouped) groupCount = 0;
  ...
  const initialGroups = BOOK_SOLO_TYPES.includes(kind) && !grouped
    ? []
    : soloMode
    ? soloRows
    : groupMode === "base"
...
export async function renameBookActivity(actId, { title, topic, bookUrl }, activity = null) {
  ...
  return updateBookActivity(actId, {
    title: nextTitle,
    topic: nextTopic,
