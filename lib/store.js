...
export async function addStudyBoard(
  user,
  { title, type = "student", description = "", keyword = null, classId = null }
) {
  const data = {
...
export async function addStudyCard(user, boardId, { title = "", content, imageUrl = null }) {
  const data = {
    boardId,
    title: title ?? "",
    content,
...
export async function updateStudyCard(boardId, cardId, { title, content, imageUrl }) {
  const patch = { title: title ?? "", content, imageUrl: imageUrl ?? null };
  if (isFirebaseConfigured) {
