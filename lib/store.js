...
const mock = {
  ...
  notices: [
    {
      ...
      title: "질문 게시판 이용 안내",
      content:
        "화학 주제별 키워드를 선택해서 질문을 올려 주세요. 이해된 내용은 한 줄 정리(회고)로 남겨 두면 나중에 복습할 때 도움이 됩니다.",
      authorId: "teacher_01",
      authorName: "선생님",
  ...
  kwl: [],
  // 접속 핑 — 10분 버킷 단위 (classId + userId + bucket)
  presence: [],
};
...
const mockListeners = {
  ...
  presence: new Map(),   // `${classId}_${userId}` -> Set(callback)
};
...
// 헷갈리는 글자(0/O, 1/I)를 뺀 입장 코드 생성 (6자리)
...
export async function addClass(user, name) {
  ...
  const data = { name: name.trim(), joinCode, createdBy: user.uid };
  if (isFirebaseConfigured) {
  ...
  const created = { id: `cl${++mockSeq}_m`, ...data, createdAt: new Date() };
  mock.classes.push(created);
  notifyClasses();
  return created;
}

// 입장 코드로 반 찾기 — 학생 입장 화면에서 사용. 없으면 null.
export async function findClassByCode(code) {
  ...
  return mock.classes.find((c) => c.joinCode === norm) ?? null;
}
...
export async function addStudyCard(user, boardId, { title = "", content, imageUrl = null, attachments = [] }) {
  const data = {
    ...
    authorStudentId: user.studentId ?? null,
  };
  if (isFirebaseConfigured) {
    // 문서 ID를 작성자 uid로 고정 → "보드당 카드 1개"를 문서 수준에서 보장.
    // 동시 탭/재요청에도 새 카드가 늘지 않고 같은 문서를 덮어씁니다(중복 제출 방지).
  ...
  // mock도 동일 의미: 같은 보드에 내 카드가 이미 있으면 교체, 없으면 추가
  const existing = mock.studyCards.find(
    (c) => c.boardId === boardId && c.authorId === user.uid
  );
  if (existing) {
...
export async function addKwl(classId, user, date, { K = "", W = "", L = "" }) {
  ...
  if (isFirebaseConfigured) {
    await addDoc(collection(db, "kwl"), {
      ...data,
      createdAt: serverTimestamp(),
    });
    return;
  }
  mock.kwl.push({ id: `kwl_${++mockSeq}`, ...data });
...
// ─── 접속 기록(presence) — 10분 버킷 단위 ──────────────────────
// 페이지가 열려 있는 동안 주기적으로 호출. 같은 10분 버킷은 문서 하나만
// 덮어써(setDoc) "이 10분에 접속함"을 1건으로 기록합니다(중복 쓰기 방지).
function tenMinBucket(when = new Date()) {
  const d = new Date(when);
  d.setMinutes(Math.floor(d.getMinutes() / 10) * 10, 0, 0);
  return d;
}

function notifyPresence(classId, userId) {
  // 반+사용자 구독자
  const key = `${classId}_${userId}`;
  notify(
    mockListeners.presence.get(key) ?? new Set(),
    mock.presence
      .filter((p) => p.classId === classId && p.userId === userId)
      .sort((a, b) => new Date(a.bucket) - new Date(b.bucket))
  );
  // 사용자 전체(반 무관) 구독자 — 관리자 대시보드용
  const userKey = `user_${userId}`;
  notify(
    mockListeners.presence.get(userKey) ?? new Set(),
    mock.presence
...
export async function recordPresence(classId, user, when = new Date()) {
  if (!classId || !user) return;
  const bucketIso = tenMinBucket(when).toISOString();
  const data = {
    classId,
    userId: user.uid,
    bucket: bucketIso,
  ...
  if (isFirebaseConfigured) {
    // 문서 ID = uid_버킷 → 같은 10분은 1개 문서로 덮어쓰기 (중복 쓰기 방지)
    await setDoc(doc(db, "presence", `${user.uid}_${bucketIso}`), {
      ...data,
      lastSeen: serverTimestamp(),
    });
    return;
  }
  const id = `${user.uid}_${bucketIso}`;
  const existing = mock.presence.find((p) => p.id === id);
  if (existing) {
    existing.lastSeen = new Date();
    return;
  }
  mock.presence.push({ id, ...data, lastSeen: new Date() });
  notifyPresence(classId, user.uid);
}

// 내 접속 기록 구독 (버킷 오름차순)
export function subscribeMyPresence(classId, userId, callback) {
  if (isFirebaseConfigured) {
    const q = query(
  ...
  const key = `${classId}_${userId}`;
  if (!mockListeners.presence.has(key)) mockListeners.presence.set(key, new Set());
  const cb = () =>
    callback(
      mock.presence
        .filter((p) => p.classId === classId && p.userId === userId)
  ...
  mockListeners.presence.get(key).add(cb);
  cb();
  return () => mockListeners.presence.get(key)?.delete(cb);
}

// 특정 사용자의 접속 기록 구독 (반 무관) — 관리자 대시보드에서 학생 조회용
...
// ─── 관리자 전용: 학생 프로필 수정 ───────────────────────────────
// 해당 uid로 작성된 모든 questions·answers·공부방 카드의 author 필드를 갱신합니다.
// 카드는 studyBoards/{boardId}/cards 서브컬렉션이므로 collectionGroup("cards")로 조회합니다.
...
// ─── 관리자 전용: 학생 탈퇴 처리 ────────────────────────────────
// 해당 uid의 모든 authored content(질문·답변·공부방 카드·KWL)를 삭제합니다.
