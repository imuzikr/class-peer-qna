...
export function subscribeQuestionSignals(classId, callback) {
  if (!classId) { callback([]); return () => {}; }
  if (isFirebaseConfigured) {
    const q = query(collection(db, "questionSignals"), where("classId", "==", classId));
    return onSnapshot(
      q,
...
export async function setQuestionSignal(classId, user, active) {
  ...
  const id = `${user.uid}_${classId}`;
  if (isFirebaseConfigured) {
    const ref = doc(db, "questionSignals", id);
    if (!active) {
      await deleteDoc(ref);
      return;
    }
...
export async function addBookActivity(
  ...
  for (let i = 1; i <= groupCount; i++) {
    mock.bookGroups.push({
      id: `group_${i}`, activityId: id, groupIndex: i, groupName: nameAt(i),
      memberUids: [], members: [], leaderUid: null, retired: false,
    });
  }
...
export async function composeBookGroups(user, actId, groups) {
  if (isFirebaseConfigured) {
  ...
  groups.forEach((g) => {
    ...
    const fields = {
      groupIndex: g.index, groupName: g.name, memberUids: g.memberUids,
...
// [교사] 학생을 대신해 그 학생의 기록에 씁니다(지금은 마인드맵 교사 편집에서만
// 씀). saveParatextEntry와 달리 문서 ID·authorId를 호출자(user.uid)가 아니라
// targetUid로 고정합니다 — 그래야 교사가 써도 학생 자신의 제출물로 남고,
// 방송·진행 표시(누구 것인지)가 그대로 유지됩니다. 실제 허용 여부는
// firestore.rules가 담당 교사인지 다시 확인합니다.
export async function saveParatextEntryFor(actId, targetUid, authorName, answers) {
  const data = {
    activityId: actId,
    authorId: targetUid,
    authorName: authorName || "이름 미설정",
    answers,
  };
  if (isFirebaseConfigured) {
    await setDoc(
      doc(db, "bookActivities", actId, "entries", targetUid),
      { ...data, updatedAt: serverTimestamp() },
      { merge: true }
    );
    return;
  ...
  ensureMockBooks();
  const found = mock.bookEntries.find(
    (e) => e.activityId === actId && e.authorId === targetUid
  );
  if (found) Object.assign(found, data, { updatedAt: new Date() });
  else mock.bookEntries.push({ id: targetUid, ...data, updatedAt: new Date() });
  notifyBookEntries(actId);
}
