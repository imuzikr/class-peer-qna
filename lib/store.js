...
import { CELL_COUNT as CONSONANT_CELL_COUNT } from "./consonants";
import {
  emptyKwlsAnswers,
...
export async function startClassAttendance(classId, date = todayDateKey()) {
  ...
  const cls = mock.classes.find((c) => c.id === classId);
  if (cls) {
    cls.attendanceOpen = true;
    cls.attendanceOpenDate = date;
  }
  notifyClasses();
}
...
// 만들면서 정한 활동은 '열린' 상태로 시작합니다 — 아직 진행 중인 수업이
// 없어 잠가 둘 이유가 없고, 반대로 잠긴 채로 시작하면 학생이 프로젝트를
// 열자마자 아무것도 못 씁니다. (수업 도중 추가하는 활동은 지금까지처럼
// nextActivityLocks가 잠근 채로 시작시킵니다.)
...
export async function duplicateStudyBoard(board, targetClassId, user) {
  const data = {
    ...
    // 활동별 잠금·활동 유형(개별/모둠)도 함께 옮깁니다 — 예전엔 빠져 있어서
    // 모둠 프로젝트를 복제하면 개별 활동으로 되돌아갔습니다.
    activityLocks: board.activityLocks ?? [],
...
// 여러 반의 소속 학생을 반별로 묶어 구독 → { [classId]: uid[] }
// (출석 관리의 '반별 목록'처럼 지금 보는 반 하나가 아니라 여러 반을 한 번에
//  봐야 할 때 씀. 규칙상 교사는 memberships를 통째로 나열할 수 없어
//  subscribeClassMembers처럼 반별 where로 나눠 구독합니다)
export function subscribeMembersForClasses(classIds, callback) {
  const ids = [...new Set((classIds || []).filter(Boolean))];
  if (ids.length === 0) { callback({}); return () => {}; }
  if (isFirebaseConfigured) {
    const byClass = {};
    const emit = () => callback({ ...byClass });
    const unsubs = ids.map((cid) =>
      onSnapshot(
        query(collection(db, "memberships"), where("classId", "==", cid)),
        (snap) => { byClass[cid] = snap.docs.map((d) => d.data().uid).filter(Boolean); emit(); },
        () => { byClass[cid] = []; emit(); }
      )
    );
    return () => unsubs.forEach((u) => u());
  ...
  const emit = () => {
    const byClass = {};
    ids.forEach((cid) => {
      byClass[cid] = mock.memberships.filter((m) => m.classId === cid).map((m) => m.uid);
    });
    callback(byClass);
  };
  mockListeners.classMembers.add(emit);
...
// (누적 총계는 rewards에 그대로 있고, 과일 주기 모달과 리포트가 씁니다)
//
...
// 여러 반의 출석 기록을 반별로 묶어 구독 → { [classId]: record[] }
// (출석 관리 '캘린더형'의 반별 목록에서 씀 — 지금 보는 반 하나가 아니라
//  교사가 가진 반 전체를 한 번에 봐야 하므로, 반별 구독을 반 수만큼
//  펼쳐서 합칩니다)
export function subscribeAttendanceForClasses(classIds, callback) {
  const ids = [...new Set((classIds || []).filter(Boolean))];
  if (ids.length === 0) { callback({}); return () => {}; }
  if (isFirebaseConfigured) {
    const byClass = {};
    ...
    const unsubs = ids.map((cid) =>
      onSnapshot(
        collection(db, "classes", cid, "attendanceRecords"),
        (snap) => { byClass[cid] = snap.docs.map((d) => ({ id: d.id, ...d.data() })); emit(); },
        () => { byClass[cid] = []; emit(); }
      )
    );
  ...
  const emit = () => {
    const byClass = {};
    ids.forEach((cid) => {
      byClass[cid] = mock.attendanceRecords.filter((r) => r.classId === cid);
    });
    callback(byClass);
  };
  ids.forEach((cid) => {
    if (!mockListeners.attendanceRecords.has(cid)) mockListeners.attendanceRecords.set(cid, new Set());
    mockListeners.attendanceRecords.get(cid).add(emit);
  });
  emit();
  return () => ids.forEach((cid) => mockListeners.attendanceRecords.get(cid)?.delete(emit));
}
...
const LESSON_MEMO_HISTORY = 50;

export function subscribeLessonMemos(classId, callback) {
  if (!classId) { callback([]); return () => {}; }
  if (isFirebaseConfigured) {
    return onSnapshot(
      query(
...
export async function addLessonMemo(classId, user, text) {
...
export async function updateLessonMemo(classId, memoId, text) {
  const body = String(text ?? "").trim().slice(0, LESSON_MEMO_MAX);
  if (!classId || !memoId || !body) return;
  if (isFirebaseConfigured) {
    await updateDoc(doc(db, "classes", classId, "lessonMemos", memoId), { text: body });
    return;
  }
...
export async function saveKwlsActivityEntry(activity, user, answers) {
...
// =============================================================
export const BOOK_GROUP_MODES = ["solo", "teacher", "random", "free"];
...
export async function addBookActivity(
  ...
  const soloMode = !BOOK_SOLO_TYPES.includes(kind) && groupMode === "solo";
  ...
  const initialGroups = BOOK_SOLO_TYPES.includes(kind)
    ? []
    : soloMode
    ? soloRows
...
// ─── 닿소리 진행률 ──────────────────────────────────────────
// [교사] 활동 하나의 진행 상황 — 화면에는 '14칸을 다 채운 학생 수'를 씁니다.
//
// 세는 단위는 낱말이 아니라 **칸**입니다. 한 칸에 낱말을 셋 넣어도 그 칸은
// 하나로 셉니다(13칸에 낱말 14개면 13칸 달성). 낱말 수로 세면 한 칸에 몰아
// 넣은 학생이 골고루 채운 학생보다 앞서 보입니다.
//
// 분모는 그 활동에 배정된 학생 전원입니다 — 한 칸도 안 채운 학생도 들어갑니다.
// 그러지 않으면 두 명만 참여한 반이 100%로 보입니다.
//
// 평균(totalFilled)도 함께 돌려줍니다 — 화면의 주된 숫자는 완료 인원이지만,
// 아무도 다 못 채운 초반에는 0/25만 보여 반이 멈춘 것처럼 읽힙니다. 그때
// '반이 어디까지 왔나'를 말풍선으로 덧붙이는 데 씁니다. 이미 읽어 온 자료에서
// 나오는 값이라 따로 드는 비용은 없습니다.
//
// [왜 구독이 아니라 한 번 읽고 캐시하나]
// 칸이 채워졌는지는 낱말 문서에만 있어서, 이 값을 구하려면 활동의 낱말을
// 전부 읽어야 합니다(학생 26명이면 400건 안팎). 목록 화면은 활동 카드가
// 계속 쌓이는 곳이라 카드마다 실시간 구독을 걸면 화면을 열 때마다 수천 건이
// 됩니다. 그래서 한 번만 읽고 세션 동안 활동별로 캐시하며, 교사가 그 활동을
// 열 때 그 활동의 캐시만 버립니다(안에서 수업하고 나오면 새 값을 봐야 하므로).
const consonantProgressCache = new Map();

export function invalidateConsonantProgress(actId) {
  consonantProgressCache.delete(actId);
}

export async function fetchConsonantProgress(actId) {
  if (!actId) return null;
  if (consonantProgressCache.has(actId)) return consonantProgressCache.get(actId);
  const result = await readConsonantProgress(actId);
  consonantProgressCache.set(actId, result);
  return result;
}

async function readConsonantProgress(actId) {
  // uid → 그 사람이 낱말을 넣은 칸(cellKey) 집합
  const cells = new Map();

  if (isFirebaseConfigured) {
    const gsnap = await getDocs(collection(db, "bookActivities", actId, "groups"));
    const groups = gsnap.docs.filter((d) => d.data().retired !== true);
    // 명단을 먼저 깔아 둡니다 — 아직 아무것도 안 쓴 학생도 0칸으로 평균에 듭니다.
    groups.forEach((g) => {
      const d = g.data();
      (d.members ?? []).forEach((m) => { if (m?.uid) cells.set(m.uid, new Set()); });
      (d.memberUids ?? []).forEach((u) => { if (u && !cells.has(u)) cells.set(u, new Set()); });
    });
    const lists = await Promise.all(
      groups.map((g) => getDocs(collection(g.ref, "words")))
    );
    lists.forEach((ws) =>
      ws.forEach((w) => {
        const d = w.data();
        // 명단에 없는 사람(반에서 빠진 학생)이 남긴 낱말은 세지 않습니다 —
        // 교사 화면의 '학생별 진행'이 명단으로 거르는 것과 같은 기준입니다.
        if (d.authorId && d.cellKey && cells.has(d.authorId)) {
          cells.get(d.authorId).add(d.cellKey);
        }
      })
    );
  } else {
  ...
  const filled = [...cells.values()].map((s) => s.size);
  return {
    students: filled.length,
    // 14칸을 다 채운 학생 수 — 카드에 내놓는 숫자
    done: filled.filter((n) => n >= CONSONANT_CELL_COUNT).length,
    // 말풍선용 — 평균(합계 ÷ 사람 수)과 가장 많이 채운 칸 수
    totalFilled: filled.reduce((a, b) => a + b, 0),
    best: filled.reduce((a, b) => Math.max(a, b), 0),
  };
}
