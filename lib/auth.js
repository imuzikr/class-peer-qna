...
export async function signInWithGoogle(requestedRole = null) {
  ...
  // 미리 막을 수 없습니다. 신규 계정인데 학교 도메인도 최고 관리자
  // 이메일도 아니면, 방금 만들어진 Auth 계정을 되돌립니다(그대로 두면
  // 프로필 없는 계정이 남아 다음에도 로그인은 되는데 앱은 못 쓰는
  // 어정쩡한 상태가 됩니다).
  if (isNewUser && !isBootstrapAdminEmail(cred.user.email) && !isSchoolEmail(cred.user.email)) {
    try {
