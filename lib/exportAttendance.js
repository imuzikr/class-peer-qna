...
// 라이브러리 없이 SpreadsheetML을 써서, 설치할 것 없이 바로 열립니다
...
export function attendanceFileName(className, dates = []) {
  ...
  return `출석부_${cls}${last ? `_${last}` : ""}.xls`;
}

export function downloadAttendanceWorkbook({ className = "", roster = [], records = [] } = {}) {
  const { headers, rows, dates } = buildAttendanceSheet(roster, records);
  downloadWorkbook(
