...
// 학교 구글 워크스페이스 계정 이름("30105 홍길동" — 학번 5자리 + 공백 + 이름
// 2~10자)을 학번과 이름으로 분리합니다. 형식이 다르면 null 반환.
export function splitWorkspaceName(raw) {
  const m = /^(\d{5})\s+(\S{2,10})$/.exec(String(raw ?? "").trim());
  return m ? { studentId: m[1], realName: m[2] } : null;
}
