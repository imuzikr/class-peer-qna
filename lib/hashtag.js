// =============================================================
// 열 개의 해시태그 — 한 학생의 기록 모양과 셈 (순수 함수)
// -------------------------------------------------------------
// 학생이 책·기사·잡지·뉴스레터를 읽고, 글을 꿰뚫는 낱말을 해시태그로
// 뽑습니다(최대 열 개). **먼저 '내가 찾은 해시태그'를 칩으로 모으고**, 그
...
// 못 채워도 공개 대상입니다 — 목록과 슬라이드 머리에 n/10으로 적습니다.
...
// 생각을 씁니다. 저장 모양은 그대로 열 칸(tags[10])이고 칩 하나가 칸
...
// 칩 더하기 — 같은 태그(대소문자 무시)는 막고, 열 칸이 차면 넘친 것을
// 돌려줍니다. 화면은 dupes·overflow로 까닭을 적습니다.
export function addTagsTo(tags, raw) {
  ...
  splitTags(raw).forEach((tag) => {
    ...
    const slot = list.findIndex((e) => !isEntryStarted(e));
...
// 칩 빼기 — 그 칸을 통째로 빼고 맨 뒤에 빈 칸을 하나 채웁니다(늘 열 칸).
export function removeTagAt(tags, index) {
  const list = normalizeHashtagPost({ tags }).tags;
  if (!(index >= 0 && index < list.length)) return list;
  return [...list.filter((_, i) => i !== index), emptyEntry()];
}
...
export function normalizeHashtagSlide(raw) {
  ...
  const s = r.source ?? {};
  const im = r.image ?? {};
  const list = Array.isArray(r.entries) ? r.entries.slice(0, HASHTAG_COUNT) : [];
  return {
