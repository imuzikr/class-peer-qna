...
export const PARATEXT_SECTION_COUNT = PARATEXT_SECTIONS.length; // 8 (CATAPULT)
...
// ── 단계별 열기 ────────────────────────────────────────────────
// 공부방 프로젝트의 활동 잠금(studyBoards.activityLocks)과 같은 생각입니다:
// 교사가 수업 진도에 맞춰 한 단계씩 열어 줍니다.
//
// [저장 모양] bookActivities/{id}.sectionLocks = { [단계 key]: true }
//   true = 잠김. 공부방은 배열(activityLocks[i])인데, 그건 활동 이름이 바뀌고
//   순서가 움직이기 때문입니다. 여기 여덟 단계는 이 파일에 못 박힌 고정
//   목록이라 key로 잡습니다 — 답(answers)을 key로 저장하는 것과 같은 이치로,
//   나중에 단계를 끼워 넣어도 어긋나지 않습니다.
//
// [없으면 잠김] 키가 없거나 필드 자체가 없으면 잠긴 것으로 봅니다. 공부방에서
//   새 활동이 잠긴 채로 시작하는 것과 같습니다 — 교사가 열어 주는 것이
//   이 기능의 전부라, '모르면 열림'이면 열어 주는 일 자체가 없어집니다.
//   활동을 만들 때 1단계만 열어 둡니다(lib/store.js의 initialSectionLocks).
//
// [규칙(firestore.rules)에는 넣지 않습니다] 여덟 단계의 답이 entries/{uid}
//   한 문서 한 맵에 다 들어 있어, 규칙이 '3단계 필드만 안 바뀌었나'를 보려면
//   필드 이름 15개를 규칙 파일에 베껴 두고 손으로 맞춰야 합니다. 이 파일이
//   단계의 유일한 출처라는 원칙이 깨집니다. 화면에서만 막습니다 — 공부방도
//   같습니다. 활동 전체를 잠그는 locked는 그대로 규칙이 막고 있어서,
//   '수업 끝, 이제 아무도 못 고침'은 여전히 규칙이 보장합니다.
export function isSectionLocked(activity, sectionKey) {
  return (activity?.sectionLocks ?? {})[sectionKey] !== false;
}

// 열려 있는 단계 수 — 교사 화면의 '열린 단계 n/8'에 씁니다.
export function openSectionCount(activity) {
  return PARATEXT_SECTIONS.filter((s) => !isSectionLocked(activity, s.key)).length;
}

// 지금 상태에서 한 단계를 열거나 닫은 새 맵을 만듭니다.
// 저장할 때 여덟 키를 모두 적어 둡니다 — 반쯤 적힌 맵이 남으면 '없으면 잠김'
// 규칙 때문에 화면마다 다르게 읽힐 여지가 생깁니다.
export function sectionLocksWith(activity, sectionKey, locked) {
  const next = {};
  for (const s of PARATEXT_SECTIONS) {
    next[s.key] = s.key === sectionKey ? locked : isSectionLocked(activity, s.key);
  }
  return next;
}

// 앞에서부터 n단계까지 열린 맵 — '다음 단계 열기'와 '모두 열기/닫기'에 씁니다.
export function sectionLocksUpTo(n) {
  const next = {};
  PARATEXT_SECTIONS.forEach((s, i) => { next[s.key] = i >= n; });
  return next;
}

// 아직 잠겨 있는 첫 단계의 자리 — 없으면 -1(다 열림).
// 교사가 건너뛰며 열었어도 '아직 안 연 것 중 가장 앞'을 집습니다.
export function firstLockedIndex(activity) {
  return PARATEXT_SECTIONS.findIndex((s) => isSectionLocked(activity, s.key));
}

// 활동을 만들 때 쓸 처음 상태 — 1단계만 열어 둡니다.
export function initialSectionLocks() {
  return sectionLocksUpTo(1);
}

// ── 전광판·진행 패널이 함께 쓰는 단계 정의 ────────────────────
// 두 화면(모달 전광판 · 작업 화면 오른쪽 패널)이 같은 줄과 같은 칸 색을
// 써야 하므로, 정의를 여기 한 곳에 둡니다 — 한쪽만 고치면 같은 학생이
// 두 화면에서 다른 상태로 보입니다.
export function paratextRows(activity) {
  return PARATEXT_SECTIONS.map((s) => ({
    key: s.key,
    letter: s.letter,
    label: s.ko,
    hint: s.prompt,
...
// 한 단계에서 그 학생이 채운 칸 수 / 전체 칸 수 (제목 5칸 · 목차 3칸 · 머리말 2칸)
export function paratextFieldCount(section, answers = {}) {
...
// 그다음이 잠김. 공부방 전광판의 cellState와 같은 차례입니다.
export function paratextCellState(row, answers = {}) {
