...
export const BOOK_ACTIVITY_TYPES = ["consonant", "paratext", "raft", "kwls", "mindmap", "opinion", "hashtag"];
...
// 없습니다. '내 생각은요...'(메모는 opinionNotes 아래)와 '열 개의 해시태그'
// (보고서는 hashtagPosts 아래 — 교사가 공개하기를 누른 뒤에 서로 읽음)입니다.
...
// 해시태그는 보고서·댓글 구독이 한 묶음으로 깨어납니다(데모 모드라 가볍습니다)
...
export async function addBookActivity(
  ...
  // 열 개의 해시태그 — 친구 보고서는 교사가 '공개하기'를 누른 뒤에야 보입니다
...
export async function purgeBookActivity(actId) {
  if (isFirebaseConfigured) {
    ...
    // '열 개의 해시태그'의 보고서와 댓글 — 보고서의 그림 파일도 함께 치웁니다
...
//
// [읽기] 공개 전(published가 참이 아님)에는 제 보고서와 제 보고서에 달린
...
export function subscribeMyHashtagPost(actId, uid, callback) {
  ...
  if (isFirebaseConfigured) {
    return onSnapshot(
      ...
      (snap) => callback(snap.exists() ? { id: snap.id, ...snap.data() } : null),
      (e) => { console.warn("[책방] 내 해시태그 보고서를 읽지 못했어요:", e?.code, e?.message); callback(null); }
    );
  }
...
// 반 전체 보고서 — 교사는 늘, 학생은 **공개된 뒤에만** 부릅니다(그 전에는
...
export function subscribeHashtagPosts(actId, callback) {
  ...
  if (isFirebaseConfigured) {
    return onSnapshot(
      ...
      (snap) => callback(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
      (e) => { console.warn("[책방] 해시태그 보고서를 읽지 못했어요:", e?.code, e?.message); callback([]); }
    );
  }
  return hashtagEmit(actId, () => callback(mock.bookHashtagPosts.filter((p) => p.activityId === actId)));
}

// [학생] 내 보고서 저장 — 같은 문서에 덮어씁니다(자동 저장이라 자주 불립니다).
...
// [학생] 내 보고서에 달린 댓글을 여기까지 봤다 — '새 댓글 n'을 끕니다.
// 보고서가 아직 없으면(댓글이 달릴 수도 없으므로) 아무것도 안 합니다.
...
// 댓글 — postUid를 주면 그 보고서의 것만(학생이 제 보고서를 보거나, 공개 뒤
// 친구 보고서 하나를 열 때), 안 주면 활동 전부(교사 · 공개 뒤 목록의 댓글 수).
...
// 댓글 지우기 — 쓴 사람과 담당 교사만(보고서 주인은 못 지웁니다 — 규칙).
...
// [교사] 친구 보고서 공개하기 / 공개 거두기 — 활동 전체에 한 번에.
