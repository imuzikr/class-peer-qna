...
//
// 짝짓기는 `matchActivitySections`에 맡깁니다 — 교사가 활동 이름을 고친 뒤라도
// 학생이 쓴 내용이 제자리에 남습니다(그 함수의 주석 참고).
export function appendToActivity(card, activities, index, appendHtml) {
  const acts = activities ?? [];
  if (index < 0 || index >= acts.length) return null;
  const paired = matchActivitySections(card, acts);
  // 제목은 '카드에 박혀 있던 것'보다 **지금 활동 이름**을 씁니다 — 이 카드를
  // 다시 저장하는 김에 이름이 바뀐 칸도 따라오게.
  const titles = acts.map((name) => name);
  const contents = acts.map((_, i) => paired[i]?.content ?? "");
