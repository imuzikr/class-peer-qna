...
// 카드 한 장 → 활동 칸이 몇 개 채워졌는지 + 전체 글자 수.
// 학생 카드(보드 목록)에 진행 상태를 작게 미리 보여 줄 때 씁니다.
// '채워짐'은 제출 판정(DONE_MIN_CHARS)보다 낮은 기준(1자 이상)입니다 —
// 여기서는 "얼마나 충분히 썼는가"가 아니라 "칸을 건드리기라도 했는가"를
// 보여 주는 용도라, 한 글자만 써도 채운 것으로 셉니다.
export function cardActivitySummary(card, activities) {
  ...
  return {
    ...
    filled: lengths.filter((n) => n > 0).length,
    chars: lengths.reduce((sum, n) => sum + n, 0),
    // 활동 순서대로 "칸을 건드렸는가" — 카드 미리보기의 칸 색을 정확한
    // 자리에 칠하려면 개수만으론 부족해 순서 정보가 필요합니다.
    segments: lengths.map((n) => n > 0),
  };
}
