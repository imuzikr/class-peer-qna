...
// 앞 글자의 받침에 따라 조사를 고릅니다 — '구름가 되어'가 아니라 '구름이 되어'.
// 한글이 아닌 말(영문·숫자)이나 빈칸은 어느 쪽인지 정할 수 없어 '이(가)'로 둡니다.
function withJosa(word, afterBatchim, afterVowel) {
  const code = word.charCodeAt(word.length - 1);
  if (Number.isNaN(code) || code < 0xac00 || code > 0xd7a3) {
    return `${word}${afterBatchim}(${afterVowel})`;
  }
  // 한글 음절은 (초성×21 + 중성)×28 + 종성 구조 — 나머지가 0이면 받침이 없습니다
  const hasBatchim = (code - 0xac00) % 28 !== 0;
  return `${word}${hasBatchim ? afterBatchim : afterVowel}`;
}
...
export function raftSentence(answers = {}) {
  ...
  return `나는 ${withJosa(role, "이", "가")} 되어, ${audience}에게, ${format} 형식으로, ${topic}에 대해 씁니다.`;
}
