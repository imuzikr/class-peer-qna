    ...
    <div className="study-project-dash">
      <div className="study-project-dash-head">
        ...
        {/* 프로젝트는 원본(선생님의 것)으로 만들어집니다 — 반에서는 '수업관리'
            창의 프로젝트 탭에서 '우리 반에 가져오기'로 엽니다. 만들고 나면 그
      ...
      {projects.length === 0 ? (
        <p className="empty-note">
          {isTeacher
            ? "이 반에 연 프로젝트가 아직 없어요. ‘수업관리’의 프로젝트 탭에서 골라 ‘우리 반에 가져오기’를 누르거나, ‘＋ 프로젝트 만들기’로 새로 만들어 보세요."
      ...
      <button type="button" className="study-project-open" onClick={onOpen}>
        <span className="study-project-badges">
          <span className={`study-project-badge${isGroup ? " group" : ""}`}>
            {isGroup ? <><IconGroup size={15} /> 모둠</> : <><IconIndividual size={15} /> 개별</>}
          </span>
          {activities.length > 0 && (
